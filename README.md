...
2. [Solution Overview](https://gitlab.aws.dev/jiyukim/automation-of-building-a-transactional-data-lake/-/blob/develop/README.md#solution-overview)
...
## Introduction
This solution provides a simple and deployable use case for automating the process of building a transactional data lake on the AWS environment. With this solution, you will configure the parameters of AWS Glue, including choosing an open table format among Apache Hudi, Apache Iceberg, and Delta Lake. Once the parameters are configured, you will deploy this solution on your AWS environment and initiate the automated process of building a transactional data lake using the chosen open table format.
<br>
<br>
## Solution Overview
#### Background
...
This solution aims to mitigate the above challenges that you may encounter. We encourage you to use this solution as a high-level guidance which can help you to understand how a transactional data lake is built on AWS environment with each open table formats, and eventually to identify what open table format is likely a good fit for your use case. 

#### Proposed Solution
<!--[Image] Architecture Diagram-->
![Architecture Diagram](./images/aws-smaples.jpg)

The focus area of the solution is to help customers understand how to build a transactional data lake with their preferred open table format. To simplify the solution and save costs, we make the following assumptions:
...
With the above assumptions, the solution will only provision the cloud resources enclosed with AWS CDK box on the above architecture diagram image as default.
...
This solution will set up the following cloud resources when you choose **Apache Hudi** or **Apache Iceberg**:
...
This solution will set up the following resources when you choose **Delta Lake**:
|Resource|Description|
|--|--|
|S3 Bucket(1)|Object Storage for Raw Data Lake|
|S3 Bucket(2)|Object Storage for Transactional Data Lake|
...
#### 6. Check the Results
To verify that the solution is working correctly, you can perform the following activities: 
